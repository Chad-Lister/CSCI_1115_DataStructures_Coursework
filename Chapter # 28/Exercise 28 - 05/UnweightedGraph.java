...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public List<Integer> getPath(int u, int v) {
		...
		//  If input is bad (ie; blanks), return empty list.
		if (u == -1 || v == -1) {
			List<Integer> pathList = new ArrayList<>();
			return pathList;
		}
		
		//  Invoke bfs(u) for tree.
		SearchTree pTree = bfs(u);
...
//		else {
		...
		pathList.add(v);
		//  get parent.
		int t = pTree.getParent(v);
		//  While not end of tree.
		while (t != -1) {
			//  Add parent of v.
			pathList.add(t);
			//  Make parent of t new parent and loop through tree.
			t = pTree.getParent(t);
		}
	//	}

		//  Check for path correctness using their method.
		System.out.println();
		System.out.println("  Path using bfs.getPath() method.  ");	
		System.out.println(pTree.getPath(v));
		System.out.println();
